...
import { useGetAllBrandsQuery } from "@/redux/brand/brandApi";
import ReactSelect, { SingleValue, GroupBase } from "react-select";
import ErrorMsg from "../../common/error-msg";
import Loading from "../../common/loading";
import ProductType from "./product-type";
...
const ProductTypeBrand = ({
  ...
  if (!isLoading && !isError && brands?.success) {
    ...
    const handleBrandChange = (selectBrand: string) => {
      const brand = brandItems.find((b) => b.name === selectBrand);
      if (brand) setSelectBrand({ id: brand._id as string, name: selectBrand });
    };

    const options: SingleValue<OptionType>[] = brandItems.map((b) => ({
      value: b.name,
      label: b.name,
    }));
        ...
        <Controller
          ...
          render={({ field }) => (
            <ReactSelect<OptionType, false, GroupBase<OptionType>>
              ...
              defaultValue={
                default_value
                  ? { label: default_value.brand, value: default_value.brand }
                  : { label: "Select..", value: "" }
              }
