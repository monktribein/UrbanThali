    ...
    if (product_items.length === 0) {
      content = (
        <div className="text-center pt-80 pb-80">
          <h3>Sorry, nothing matched <span style={{color:'#FCB53B'}}>{searchText}</span> search terms</h3>
        </div>
      );
    }
                  ...
                  <div className="tp-shop-main-wrapper">
                    ...
                    {next < product_items?.length && (
                      <div className="load-more-btn text-center pt-50">
                        <button onClick={handleLoadMore} className="tp-btn tp-btn-2" style={{backgroundColor: '#FCB53B', color: 'white', border: 'none'}}>
                          Load More
                        </button>
                      </div>
                    )}
